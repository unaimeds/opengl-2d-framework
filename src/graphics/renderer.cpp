...
Renderer::Renderer(const glm::uvec2& window_size, Texture& texture) :
...
{
    debug.info(
        "Creating renderer: {} scale, {} max objects, {} max vertices, {} max indices",
        SCALE, MAX_OBJECTS, MAX_VERTICES, MAX_INDICES
    );
    ...
    // ibo
    std::uint32_t indices[MAX_INDICES];
    std::uint32_t index_offset = 0;

    for (std::uint32_t i = 0; i < MAX_INDICES; i += 6, index_offset += 4) {
    ...
    debug.info("Creating index buffer with size {} bytes", MAX_INDICES * sizeof(std::uint32_t));
    glCreateBuffers(1, &index_buffer);
    glNamedBufferStorage(index_buffer, MAX_INDICES * sizeof(std::uint32_t), indices, 0);
...
void Renderer::draw_quad(const glm::vec2& position, const glm::vec2& size, std::uint32_t layer_index, const glm::mat4x2& uv) {
    ...
    for (std::uint32_t i = 0; i < 4; i++) {
        ...
        sprite_vertices.emplace_back(transformed_position, uv[i], (float)layer_index);
    }
...
void Renderer::draw_text(const glm::vec2& position, std::string_view text, const glm::vec4& color) { // TODO: kerning
    ...
    for (auto c : text) {
        ...
        glm::vec2 min = { glyph.offset.x / (float)texture_size.x, glyph.offset.y / (float)texture_size.y };
        glm::vec2 max = { (glyph.offset.x + glyph.size.x) / (float)texture_size.x, (glyph.offset.y + glyph.size.y) / (float)texture_size.y };
        glm::vec2 uv[] = {
                { min.x, min.y },
                { max.x, min.y },
                { max.x, max.y },
...
void Renderer::add(const glm::vec2 &position, const glm::vec2 &size, std::uint32_t texture_layer, glm::vec2 uv[4], const glm::vec4& color) {
    ...
    for (std::uint32_t i = 0; i < 4; i++) {
        auto transformed_position = modelMatrix * glm::vec4(vertex_positions[i], 0.0f, 1.0f);
        text_vertices.emplace_back(transformed_position, uv[i], color, (float)texture_layer);
    }

    text_index_count += 6;
}

void Renderer::set_vertex_array_attributes(std::uint32_t vertex_array, const std::vector<VertexAttribute>& attributes) {
    auto offset = 0u;

    for (std::uint32_t i = 0; i < attributes.size(); i++) {
        auto count = (int)attributes[i];
